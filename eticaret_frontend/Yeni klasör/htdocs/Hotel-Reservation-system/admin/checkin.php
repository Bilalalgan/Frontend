	...
	<div class = "container-fluid" style="min-height:77.6vh;">	
		<div class = "panel panel-default">
			...
			<div class = "panel-body">
				...
				<a class = "btn btn-info disabled" href = "checkin.php"><span class = "badge"><?php echo $f_ci['total']?></span>    Giriş</a>
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `guest` NATURAL JOIN `room` WHERE `status` = 'Giris'") or die(mysqli_query());
						...
						<tr>
							...
							<td><?php echo "<label style = 'color:#00ff00;'>".date("d.m.Y", strtotime($fetch['checkin']))."</label>"." @ "."<label>".date("H:i ", strtotime($fetch['checkin_time']))."</label>"?></td>
							<td><?php echo $fetch['days']?></td>
							<td><?php echo "<label style = 'color:#ff0000;'>".date("d.m.Y", strtotime($fetch['checkin']))."</label>"; ?></td>
							<td><?php echo $fetch['status']?></td>
							<td><?php if($fetch['extra_bed'] == "0"){ echo "None";}else{echo $fetch['extra_bed'];}?></td>
							<td><?php echo $fetch['bill'].".00 TL"?></td>
							<td><center><a class = "btn btn-warning" href = "checkout_query.php?transaction_id=<?php echo $fetch['transaction_id']?>" onclick = "confirmationCheckin(); return false;"><i class = "glyphicon glyphicon-check"></i> Çıkış</a></center></td>
						</tr>
						<?php
							}
						?>
...
<script type = "text/javascript">
	function confirmationCheckin(anchor){
		var conf = confirm("Çıkış yapmak istediğinizden emin misiniz?");
		if(conf){
